...
if [[ -z "${VERSION}" || -z "${CHECKSUMS}" ]]; then
  echo "missing required arguments: --version and --checksums" >&2
  usage >&2
  exit 1
fi
...
class Krust < Formula
  ...
  on_macos do
    if Hardware::CPU.arm?
      url "https://github.com/${OWNER_REPO}/releases/download/v#{version}/${DARWIN_ARM64_TGZ}"
      sha256 "${DARWIN_ARM64_SHA}"
    else
      url "https://github.com/${OWNER_REPO}/releases/download/v#{version}/${DARWIN_AMD64_TGZ}"
      sha256 "${DARWIN_AMD64_SHA}"
    end
  end

  on_linux do
    url "https://github.com/${OWNER_REPO}/releases/download/v#{version}/${LINUX_AMD64_TGZ}"
